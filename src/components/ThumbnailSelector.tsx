...
export default function ThumbnailSelector({ 
  ...
  const searchImages = async (searchQuery: string) => {
    ...
    setActualSearchQuery(searchQuery); // Show user exactly what we're searching
    ...
    try {
      const response = await fetch('/api/search-images', {
        ...
        body: JSON.stringify({ 
          query: searchQuery,  // Send exact query, no modifications
          engine: searchEngine 
        })
      });
        ...
        <div className="relative bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-4xl max-h-[80vh] overflow-hidden">
          ...
          <div className="p-6 border-b border-slate-200 dark:border-slate-700">
            ...
            <div className="flex items-center space-x-4 mb-4">
              <span className="text-sm font-medium text-slate-700 dark:text-slate-300">Search Engine:</span>
              <div className="flex space-x-2">
                <button
                  ...
                  onClick={() => {
                    ...
                    if (query.trim()) {
                      setTimeout(() => searchImages(query), 100); // Auto-search when switching engines
                    }
                  }}
            ...
            {actualSearchQuery && (
              <div className="mb-4 p-3 bg-slate-50 dark:bg-slate-700 rounded-lg">
                <div className="text-xs text-slate-500 dark:text-slate-400 mb-1">Currently searching {searchEngine === 'google' ? 'Google' : 'Bing'} for:</div>
                <div className="text-sm font-medium text-slate-700 dark:text-slate-300">"{actualSearchQuery}"</div>
                <div className="text-xs text-slate-500 dark:text-slate-400 mt-1">Safe Search: OFF</div>
              </div>
            )}
            ...
            <form onSubmit={handleSearch} className="flex space-x-3">
              <div className="flex-1 relative">
                ...
                <input
                  ...
                  placeholder="Enter your exact search terms..."
